...
* 1.5 제어문
	- 1.5.1 if 조건문 감축과 단순화
		...
		이제 객체 전용 변수에 null 포인터 대신 null 객체를 넣어준다
		// 더미 전용 null 객체를 넣어둠
		player = new NullActor();

		null 객체의 경우에는 아무것도 하지 않으므로, 객체의 존재를 확인하지 않아도 문제 없다
		// 이동
		player->move();
		... 생략
		player->draw();

		null 체크를 해야하는 설계는 되도록 피하라
		~~~

	- 1.5.2 for 반복문 감추고가 단순화
		1. for 반복문의 기본 원칙은 **1개의 작업만 반복**(2개 이상의 작업을 반복문 내부에 넣지마라)
		2. for 반복문을 사용한 알고리즘 대부분은 STL 알고리즘 함수로 변환할 수 있다
		3. for_each 함수: 모든 요소에 대해 특정한 처리를 하는 가장 간단한 반복 알고리즘 함수
		~~~ 
		// for 반복문을 사용해 actors의 모든 요소에 대해 draw함수를 호출하는 코드
		// 모든 캐릭터 렌더링
